"""Engagement tab — brand comment agent."""
import customtkinter as ctk
from widgets import AgentCard, GREEN, GREEN2, TEXT_DIM, FONT


class EngageTab(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(bar, text="◈  ETKİLEŞİM MERKEZİ",
                      font=ctk.CTkFont(family=FONT, size=9), text_color=TEXT_DIM).pack(side="left")
        ctk.CTkButton(
            bar, text="▶  Yorumları Üret", width=150, height=30,
            fg_color=GREEN2, hover_color=GREEN, text_color="#08090A",
            font=ctk.CTkFont(family=FONT, size=10, weight="bold"),
            corner_radius=0, command=lambda: self.card_engage.run(),
        ).pack(side="right")

        self.card_engage = AgentCard(self, "engage")
        self.card_engage.pack(fill="both", expand=True, padx=16, pady=(0, 16))

    def get_cards(self) -> list:
        return [self.card_engage]
